...
if default_slice
    input_slice_nr = floor(ROI_SizeX/2); % uses middle slice by default
end
fname = [fname '_YZslice_' num2str(input_slice_nr) '.tif'];
...
if fseek(vol_FID, input_slice_nr * 2, 'bof') == -1 % would need to change this if using datasets with different bit depths
...
loaded_slices = fread(vol_FID, [ROI_SizeY, ROI_SizeZ], 'uint16=>uint16', (ROI_SizeX-1)*2); % would need to change this if using datasets with different bit depths
fclose(vol_FID);
