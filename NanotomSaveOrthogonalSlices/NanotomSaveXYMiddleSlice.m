%% % This script loads the middle slice of a .vol file and stores it as a .tif
...
if default_slice
    input_slice_nr = floor(ROI_SizeZ/2); % uses middle slice by default
end
fname = [fname '_XYslice_' num2str(input_slice_nr) '.tif'];
...
if fseek(vol_FID, ROI_SizeX * ROI_SizeY * input_slice_nr * 2, 'bof') == -1 % would need to change this if using datasets with different bit depths
...
loaded_slices = fread(vol_FID, [1, ROI_SizeX * ROI_SizeY * 1], 'uint16=>uint16'); % would need to change this if using datasets with different bit depths
fclose(vol_FID);
clearvars vol_file_list vol_FID;

loaded_slices = reshape(loaded_slices, ROI_SizeX, ROI_SizeY);
